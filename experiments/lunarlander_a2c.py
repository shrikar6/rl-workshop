"""
LunarLander experiment using A2C agent.

Trains an A2C (Advantage Actor-Critic) agent on the LunarLander-v3 environment
using separate MLP policy and value networks. Policy uses a discrete action
head (4 actions); value network uses a scalar value head.
"""
...
def main():
    """Run the LunarLander A2C experiment."""
    # Configuration
    seed = 1
    num_episodes = 15000
    policy_lr = 1e-4
    ...
    tracker = Tracker(
        ...
        experiment_name="lunarlander_a2c"
    )
    ...
    print("Starting LunarLander A2C experiment")
    ...
    # Final results
    tracker.log_final(metric="return", success_threshold=200.0, window=num_episodes // 10)
